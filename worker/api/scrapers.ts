  ...
  if (pathname === '/api/scrapers/test') {
    ...
    if (sourceId != null) {
      const row = await env.DB.prepare(
        'SELECT id, kind, url, config_json FROM scraper_sources WHERE id = ?'
      ).bind(sourceId).first<ScraperSourceRow>()
  ...
  if (pathname === '/api/scrapers') {
    if (request.method === 'GET') {
      const rows = await env.DB.prepare(
        'SELECT id, kind, url, config_json, created_at FROM scraper_sources ORDER BY created_at ASC'
    ...
    if (request.method === 'POST') {
      ...
      const r = await env.DB.prepare(
        'INSERT INTO scraper_sources (kind, url, config_json) VALUES (?, ?, ?)'
      )
        .bind(kind, url, config_json)
        .run()
      const row = await env.DB.prepare(
        'SELECT id, kind, url, config_json, created_at FROM scraper_sources WHERE id = ?'
      )
