...
export async function runPipeline(db: D1Database, presetId: number): Promise<{ runId: number; passed: number; results: { title: string; link: string }[] }[]> {
  const preset = await db.prepare('SELECT id, name, config FROM filter_presets WHERE id = ?').bind(presetId).first<{ id: number; name: string; config: string }>()
  if (!preset) return []
...
export async function runAllPresets(db: D1Database): Promise<void> {
...
export async function notifyWebhook(db: D1Database, runId: number, results: { title: string; link: string }[]): Promise<void> {
